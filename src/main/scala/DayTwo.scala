object DayTwo {
  ...
  val rows: Seq[String] = input.split('|')
  val matrix: Seq[Seq[Int]] = rows.map(_.split(',').toSeq.map(_.toInt))
  ...
  val partTwo = {
    ...
    def findDivisible(row: Seq[Int]): Int = {
      val highHalf = row.sorted.drop(row.size / 2)
      val lowHalf = row.sorted.take(row.size / 2)
      val rawDivisions: Seq[Double] =
        for {
          x <- highHalf
          y <- lowHalf
        } yield (x.toDouble / y)
      ...
      rawDivisions
        .find(x => x == Math.floor(x))
        .map(_.toInt).get
    }

    matrix.map(findDivisible).sum
  }
